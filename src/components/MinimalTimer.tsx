...
export function MinimalTimer({ pomodoro, activeTask, breathBox }: MinimalTimerProps) {
  ...
  return (
    <div className="flex flex-col items-center gap-4 sm:gap-6 md:gap-8 w-full px-4">
      {/* Giant timer or BreathBox */}
      {isBreathBoxActive && breathBox ? (
        <BreathBox
      ...
      {/* Active Task Name - Below timer (only for regular pomodoro mode) */}
      {!isBreathBoxActive && activeTask && !activeTask.isCompleted && mode === 'pomodoro' && (
      ...
      <div className="flex items-center gap-3 sm:gap-4 md:gap-6">
        ...
        <Button
          onClick={() => {
            if (isBreathBoxActive && breathBox) {
